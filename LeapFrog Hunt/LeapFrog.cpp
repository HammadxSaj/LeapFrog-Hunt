...
//operator overloading have been done for the movement of the frog, the overloaded operators in the frog class.
...
//SDL has intersection fucntion has been used to check collision for the frog, crocodiles and Woods since due to the vastly diffeent moverrect
//values it doesnot correctly detect collision for the frog and the crocodiles and woods.
void LeapFrog::CheckCollision(){
    for(Unit* x: objects){
        if (SDL_bool hasintersection = SDL_HasIntersection(&frog.moverRect, &x->moverRect)){
            if ((string(typeid(*x).name()) == "4Wood") || (string(typeid(*x).name()) == "10Crocodiles")){
